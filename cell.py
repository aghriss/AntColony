...
class Cell:
    ...
    """
    draw the cell in the pygame display
    """
    def draw(self,display,block_size):
        if self.count>0:self.color = BLACK
        else:
            self.color = Colors[self.type]
        
        pygame.draw.rect(display, self.color, [self.x*block_size, self.y*block_size, block_size, block_size])
